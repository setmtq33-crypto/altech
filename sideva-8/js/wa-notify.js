// ============================================================
//  SI-DEVA — Notifikasi WhatsApp v8
...
async function waSend(text) {
  ...
  try {
    // Jika proxy terkonfigurasi, pakai proxy untuk menerima respons dan menghindari CORS
    if (c.proxyUrl) {
      const proxy = c.proxyUrl.replace(/\/$/, '');
      const resp = await fetch(proxy + '/send', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ phone: c.phone, apikey: c.apikey, text })
      });
      if (!resp.ok) {
        console.warn('[WA Notif] Proxy response not ok', resp.status);
        return false;
      }
    ...
    // Fallback: panggil CallMeBot langsung (browser tidak bisa verifikasi karena CORS)
    ...
    console.warn('[WA Notif] Gagal kirim:', e && e.message);
    return false;
  }
}
...
function waFillForm() {
  ...
  if (el('wa-proxy'))          el('wa-proxy').value            = cfg.proxyUrl|| '';
  ...
  panel.innerHTML = `
    ...
    <div style="padding:0 20px 20px;">
      ...
      <div style="margin-bottom:12px;">
        <label style="display:block;font-size:12px;color:var(--text3);margin-bottom:5px;font-weight:600;">Proxy URL (opsional)</label>
        <input type="text" id="wa-proxy" value="${cfg.proxyUrl||''}"
          placeholder="http://localhost:3000"
          style="width:100%;padding:9px 12px;border:1px solid var(--border);border-radius:6px;
                 background:var(--surface2);color:var(--text);font-size:13px;box-sizing:border-box;">
        <div style="font-size:12px;color:var(--text3);margin-top:6px;">Jika diisi, SI-DEVA akan mengirim permintaan ke proxy ini untuk mem-bypass pembatasan CORS dan menerima respons dari CallMeBot.</div>
      </div>
...
function waSaveSettings() {
  ...
  const cfg = {
    ...
    phone, apikey,
    proxyUrl:        document.getElementById('wa-proxy')?.value.trim() || '',
...
async function waTestSend() {
  ...
  // Simpan config tanpa menampilkan pesan (langsung save)
  const cfg = {
    enabled:         document.getElementById('wa-enabled')?.checked || false,
    phone, apikey,
    proxyUrl:        document.getElementById('wa-proxy')?.value.trim() || '',
