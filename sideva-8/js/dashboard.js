...
// ============================================================
//  CEGAH REFRESH TIDAK DIINGINKAN
// ============================================================

// Cegah event opd-changed memicu navigasi ulang
const originalDispatch = window.dispatchEvent;
window.dispatchEvent = function(event) {
  if (event.type === 'opd-changed') {
    const modalOpen = document.querySelector('.modal-overlay.open, .modal.open, #modal-add-user, #modal-edit-user, [id*="modal"].open');
    if (modalOpen) {
      console.log('opd-changed dicegah karena modal terbuka');
      return true; // tidak diteruskan
    }
  }
  return originalDispatch.call(this, event);
};

// Cegah reload dari event visibilitychange
document.addEventListener('visibilitychange', function() {
  ...
  if (modalOpen && document.visibilityState === 'visible') {
    console.log('visibilitychange dicegah karena modal terbuka');
    // Jangan lakukan apapun
    return;
  }
});

// Cegah reload dari event focus
window.addEventListener('focus', function() {
  const modalOpen = document.querySelector('.modal-overlay.open, .modal.open');
  if (modalOpen) {
    console.log('focus event dicegah karena modal terbuka');
    return;
  }
});

// Override showPage agar tidak bisa pindah halaman saat modal terbuka
const originalShowPage = window.showPage;
if (originalShowPage) {
  window.showPage = function(name) {
    const modalOpen = document.querySelector('.modal-overlay.open, .modal.open, #modal-add-user, #modal-edit-user');
    if (modalOpen && name !== 'dashboard') {
      console.warn('Tidak bisa pindah halaman karena modal terbuka');
      toast('Tutup modal terlebih dahulu', 'info');
      return;
    }
    return originalShowPage.call(this, name);
  };
}
...
function loadAppConfig() {
  // 1. Baca localStorage dulu untuk tampilan instan
  try {
  ...
  // 2. Async: jika sudah login, ambil config terbaru dari Supabase
  //    Ini memastikan config sinkron antar perangkat tanpa nunggu loadAllData
  if (typeof sbGetConfig === 'function' && typeof isLoggedIn === 'function' && isLoggedIn()) {
    sbGetConfig().then(cfg => {
      if (!cfg) return;
      const { _kopSuratImg, ...cfgData } = cfg;
      appConfig = { ...DEFAULT_CONFIG, ...cfgData };
      localStorage.setItem('sideva_config', JSON.stringify(cfgData));
      if (_kopSuratImg) localStorage.setItem('sideva_kop_surat_img', _kopSuratImg);
      applyAppConfig();
    }).catch(() => {});
  }
}
...
function applyAppConfig() {
  ...
  if (el3) {
    if (appConfig.singkatan) {
      el3.textContent = 'Sistem Informasi Dokumen Evaluasi Verifikas Administrasi E — Purchesing ' + s + ' ' + (appConfig.kabupaten||'') + ' secara real-time.';
    } else {
      el3.textContent = 'Sistem Informasi Dokumen Evaluasi Verifikas Administrasi E — Purchesing secara real-time untuk seluruh perangkat daerah.';
    }
  }
...
async function processKopFile(file) {
  ...
  reader.onload = async function(e) {
    ...
    try {
      ...
      // 🔥 Simpan ke konfigurasi OPD aktif
      if (typeof getCurrentOpdId === 'function' && typeof saveOpdConfig === 'function') {
        const opdId = getCurrentOpdId();
        if (opdId) {
          let opdCfg = {};
          const existing = localStorage.getItem(`sideva_opd_config_${opdId}`);
          if (existing) {
            try { opdCfg = JSON.parse(existing); } catch(e) {}
          }
          opdCfg._kopSuratImg = base64;
          await saveOpdConfig(opdId, opdCfg);
          console.log('✅ Kop surat disimpan untuk OPD', opdId);
        }
      }
    } catch(err) {
...
function hitungDeadlineCount() {
  const today = new Date(); today.setHours(0,0,0,0);
  let near = 0, overdue = 0;
  state.paket.data.forEach(p => {
    if (!p.tanggalSelesai) return;
    const selesai = new Date(p.tanggalSelesai);
    const diff = Math.ceil((selesai - today) / (1000*60*60*24));
    if (diff >= 0 && diff <= 7) near++;
    if (diff < 0) overdue++;
  });
  return { near, overdue };
}

function hitungOverBudgetCount() {
  let over = 0;
  state.paket.data.forEach(p => {
    const nego = _hitungNilaiPenetapan(p.rup);
...
function showPage(name) {
  ...
  if (name === 'laporan')    { 
    renderLaporan();
    window.dispatchEvent(new CustomEvent('sideva:open-laporan-realisasi', { detail: { info: 'Laporan Realisasi' } }));
  }
  // Dispatch event untuk rekap bidang (cek dari page active atau URL pattern)
  if (name && name.includes('rekap')) {
    window.dispatchEvent(new CustomEvent('sideva:open-rekap-bidang', { detail: { bidang: 'All' } }));
  }
...
function renderDashboard() {
  ...
  const paketData = (dashboardFilteredPaket && dashboardFilteredPaket.length) ? dashboardFilteredPaket : state.paket.data;
  const rincianData = (dashboardFilteredRincian && dashboardFilteredRincian.length) ? dashboardFilteredRincian : state.rincian.data;
  const hargaData = (dashboardFilteredHarga && dashboardFilteredHarga.length) ? dashboardFilteredHarga : state.harga.data;
  ...
  const { near, overdue } = hitungDeadlineCount();
  const overBudget = hitungOverBudgetCount();
  const totalPagu = paketData.reduce((s, p) => s + (Number(p.paguAnggaran)||0), 0);
  const totalKontrak = paketData.reduce((s, p) => s + _hitungNilaiPenetapan(p.rup), 0);
  const serapan = totalPagu > 0 ? (totalKontrak / totalPagu * 100) : 0;

  const statsHtml = `
    <div class="stat-card" onclick="drillDownStat('paket')"><div class="stat-icon-wrap">📦</div><span class="stat-label">Total Paket</span><span class="stat-value">${totalPaket}</span><span class="stat-sub">Paket pengadaan</span></div>
    <div class="stat-card" onclick="drillDownStat('rincian')"><div class="stat-icon-wrap">🧾</div><span class="stat-label">Total Item</span><span class="stat-value">${totalItem}</span><span class="stat-sub">Rincian belanja</span></div>
    <div class="stat-card" onclick="drillDownStat('paket')"><div class="stat-icon-wrap">⏰</div><span class="stat-label">Deadline ≤7 hr</span><span class="stat-value" style="color:${near ? '#f97316' : 'inherit'}">${near}</span><span class="stat-sub">Paket mendekati deadline</span></div>
    <div class="stat-card" onclick="drillDownStat('paket')"><div class="stat-icon-wrap">🚨</div><span class="stat-label">Over Budget</span><span class="stat-value" style="color:${overBudget ? '#ef4444' : 'inherit'}">${overBudget}</span><span class="stat-sub">Nilai kontrak > pagu</span></div>
    <div class="stat-card" onclick="drillDownStat('paket')"><div class="stat-icon-wrap">💰</div><span class="stat-label">Total Pagu</span><span class="stat-value" style="font-size:16px">${fmtRp(totalPagu)}</span><span class="stat-sub">Anggaran tersedia</span></div>
    <div class="stat-card" onclick="drillDownStat('paket')"><div class="stat-icon-wrap">🤝</div><span class="stat-label">Serapan</span><span class="stat-value" style="font-size:16px;color:${serapan>=80?'#22c55e':serapan>0?'#f59e0b':'#6b7280'}">${serapan.toFixed(1)}%</span><span class="stat-sub">Realisasi kontrak</span></div>
  `;
  const statsGrid = document.getElementById('stats-grid');
  if (statsGrid) statsGrid.innerHTML = statsHtml;

  const uniquePenyedia = getPenyediaTerpilih(hargaData).size;
  ...
  animateCount(document.getElementById('hero-total-harga'), hargaData.length);

  const recent = [...paketData].sort((a,b)=>b.id - a.id).slice(0,5);
  const recentContainer = document.getElementById('recent-paket');
  if (recentContainer) {
    if (!recent.length) recentContainer.innerHTML = '<div class="empty-state"><div class="empty-icon">📦</div><div class="empty-title">Belum ada data paket</div></div>';
    else recentContainer.innerHTML = `<table style="width:100%"><thead><tr><th>Nama Paket</th><th>Pagu</th><th>Bidang</th></tr></thead><tbody>${recent.map(p => `<tr><td>${strTrunc(p.namaPaket,35)}</td><td>${fmtRp(p.paguAnggaran)}</td><td>${bidangBadge(p.bidang)}</td></tr>`).join('')}</tbody></table>`;
  }

  const bidangGroups = {};
  paketData.forEach(p => { const b = p.bidang || 'Tidak Diketahui'; if (!bidangGroups[b]) bidangGroups[b] = { count:0, nilai:0 }; bidangGroups[b].count++; bidangGroups[b].nilai += Number(p.paguAnggaran)||0; });
  const maxNilai = Math.max(...Object.values(bidangGroups).map(v=>v.nilai),1);
  const summaryContainer = document.getElementById('bidang-summary');
  if (summaryContainer) {
    summaryContainer.innerHTML = Object.entries(bidangGroups).map(([b,v]) => `<div><div class="flex-between"><span>${strTrunc(b,35)}</span><span>${v.count} paket</span></div><div>${fmtRp(v.nilai)}</div><div class="progress-bar"><div class="progress-fill" style="width:${Math.round(v.nilai/maxNilai*100)}%"></div></div></div>`).join('') || '<div class="empty-state">Belum ada data</div>';
  }

  const reminderContainer = document.getElementById('dashboard-reminder-panel');
  if (reminderContainer) {
    let warnings = [];
    if (near) warnings.push(`⏰ ${near} paket akan deadline dalam ≤7 hari.`);
    if (overdue) warnings.push(`❌ ${overdue} paket sudah melewati tanggal selesai.`);
    if (overBudget) warnings.push(`🚨 ${overBudget} paket memiliki nilai kontrak melebihi pagu.`);
    if (hargaData.length === 0 && paketData.length > 0) warnings.push(`📊 Belum ada data Survey Harga.`);
    if (warnings.length) {
      reminderContainer.innerHTML = `<div class="reminder-panel"><div class="reminder-icon">⚠️</div><div class="reminder-list">${warnings.map(w => `<div>${w}</div>`).join('')}</div><button class="reminder-close" onclick="this.parentElement.style.display='none'">✕</button></div>`;
      reminderContainer.style.display = 'block';
    } else reminderContainer.style.display = 'none';
  }

  if (typeof renderCharts === 'function') renderCharts(paketData, rincianData, hargaData);
  if (typeof renderDashBottom === 'function') renderDashBottom();

  setTimeout(() => {
    if (window.chartBidangPie) window.chartBidangPie.resize();
    if (window.chartMonthlyBar) window.chartMonthlyBar.resize();
    if (window.chartPenyediaBar) window.chartPenyediaBar.resize();
    if (window.chartStatusDoughnut) window.chartStatusDoughnut.resize();
  }, 200);
...
function renderPaket() {
  ...
  if (slice.length === 0) {
    ...
    tbody.innerHTML = slice.map((p, i) => { // <-- PERUBAHAN: tambah ", i"
      const now = new Date();
      const selesai = p.tanggalSelesai ? new Date(p.tanggalSelesai) : null;
...
function filterRincian() {
  const search = (document.getElementById('search-rincian')?.value || '').toLowerCase();
  const rupSelect = document.getElementById('filter-rupdd-rincian');
  const rupInput = document.getElementById('filter-rup-rincian');
  let rup = '';
  if (rupSelect && rupSelect.value) {
    rup = rupSelect.value;
  } else if (rupInput && rupInput.value) {
    rup = rupInput.value;
  }
  ...
  // Map tahun dari paket
  const rupTahun = {};
  state.paket.data.forEach(p => {
    if (p.rup && p.tanggalPesanan) rupTahun[String(p.rup)] = new Date(p.tanggalPesanan).getFullYear();
  });

  const s = state.rincian;
  s.filtered = s.data.filter(r => {
    const matchSearch = !search || (r.itemBarang || '').toLowerCase().includes(search);
    const matchRup = !rup || String(r.rup) === String(rup);
    const matchSatuan = !satuan || r.satuan === satuan;
    let matchTahun = true;
    if (tahun) {
      matchTahun = rupTahun[String(r.rup)] === parseInt(tahun);
    }
    return matchSearch && matchRup && matchSatuan && matchTahun;
  });
  s.page = 1;
  renderRincian();
}
...
function renderRincian() {
  ...
  const slice = sorted.slice(start, start + s.perPage);

  const tbody = document.getElementById('tbody-rincian');
  if (!slice.length) {
    tbody.innerHTML = `<tr><td colspan="9"><div class="empty-state">Tidak ada data rincian</div></td></tr>`;
    document.getElementById('info-rincian').textContent = `Menampilkan 0 dari ${total} data`;
    return;
  }

  tbody.innerHTML = slice.map((r, idx) => {
    // Hitung badge harga pembanding (opsional)
    const hargaCount = (state.harga.data || []).filter(h => String(h.rup) === String(r.rup)).length;
    const badge = hargaCount >= 3 ? '<span class="badge-green">✓ 3/3</span>' : hargaCount > 0 ? `<span class="badge-yellow">~ ${hargaCount}/3</span>` : '<span class="badge-red">✗ 0/3</span>';
    return `
      <tr>
        <td class="td-mono">${start + idx + 1}</td>
        <td class="td-mono">${r.rup || '-'}</td>
        <td>${badge} ${r.itemBarang || '-'}</td>
        <td style="text-align:right">${Number(r.vol || 0).toLocaleString()}</td>
        <td>${r.satuan || '-'}</td>
        ...
        <td>
          <div style="display:flex;gap:4px">
            ...
            <button class="btn btn-danger btn-sm btn-icon" onclick="deleteRecord('rincian',${r.id},'${(r.itemBarang || '').replace(/'/g,"\\'")}')">🗑️</button>
          </div>
        </td>
      </tr>
    `;
...
function filterHarga() {
  ...
  console.log('🔍 Filter Harga - RUP:', rup, 'Status:', status, 'Tahun:', tahun);
  ...
  const s = state.harga;
  const sebelum = s.filtered.length;
  ...
  console.log(`📊 Filter Harga: ${sebelum} → ${s.filtered.length} data`);
  s.page = 1;
  renderHarga();
}
...
// Helper untuk mendapatkan OPD milik admin biasa
async function _getMyOpdIdForMaster() {
  try {
    if (typeof isSuperAdmin === 'function' && isSuperAdmin()) return null;
    ...
    return rows[0].opd_id;
  } catch(e) {
    console.warn('_getMyOpdIdForMaster error:', e);
    return null;
  }
}
...
async function renderMaster() {
  ...
  // ── OPD dengan filter untuk admin biasa ──
  let opdList = masterState.opd;
  let showOpdActions = true;
  const isSuper = (typeof isSuperAdmin === 'function' && isSuperAdmin());
  
  if (!isSuper) {
    showOpdActions = false;
    const myOpdId = await _getMyOpdIdForMaster();
    if (myOpdId) {
      opdList = masterState.opd.filter(o => String(o.id) === String(myOpdId));
    } else {
      opdList = [];
    }
  }

  const tbodyOpd = document.getElementById('tbody-opd');
  if (opdList.length === 0) {
    tbodyOpd.innerHTML = '<tr><td colspan="2" style="text-align:center;padding:20px;color:var(--text3)">Tidak ada数据</td></tr>';
  } else {
    tbodyOpd.innerHTML = opdList.map(o => `<tr>
      <td style="font-weight:600">${escapeHtml(o.namaOpd || o.namaOPD || o.nama || '-')}</td>
      <td>
        <div class="flex gap-8">
          ${showOpdActions ? `<button class="btn btn-secondary btn-sm" onclick="editMaster('opd','${o.id}')">Edit</button>` : ''}
          ${showOpdActions ? `<button class="btn btn-danger btn-sm" onclick="confirmDeleteMaster('opd','${o.id}','${(o.namaOpd||o.namaOPD||o.nama||'').replace(/'/g,"\\'")}')">Hapus</button>` : '<span class="text-muted">—</span>'}
        </div>
      </td>
    </tr>`).join('');
  }
  ...
  </table>`).join('') || '<tr><td colspan="3" style="text-align:center;padding:20px;color:var(--text3)">Tidak ada数据</td></tr>';
  ...
  document.getElementById('tbody-ppk').innerHTML = masterState.ppk.map(p => `<tr>
    ...
    <td style="text-align:center;">
      ${p.ttd ? `<img src="${p.ttd}" style="max-height:32px;max-width:90px;object-fit:contain;border:1px dashed var(--border2);border-radius:3px;background:#fff;padding:2px;" title="TTD">` : '<span style="color:var(--text4);font-size:10px;">—</span>'}
    </td>
    <td style="text-align:center;">
      ${p.cap ? `<img src="${p.cap}" style="max-height:36px;max-width:36px;object-fit:contain;border:1px dashed var(--border2);border-radius:50%;background:#fff;padding:2px;" title="Cap/Stempel">` : '<span style="color:var(--text4);font-size:10px;">—</span>'}
    </td>
    <td>
      <div class="flex gap-8">
  ...
  </tr>`).join('') || '<tr><td colspan="6" style="text-align:center;padding:20px;color:var(--text3)">Tidak ada数据</td></tr>';
  ...
  </tr>`).join('') || '<tr><td colspan="4" style="text-align:center;padding:20px;color:var(--text3)">Tidak ada数据</td></tr>';
}
...
async function savePPK() {
  ...
  const data = {
    nama, nip: v('fppk-nip'), jabatan: v('fppk-jabatan'),
  ...
  if (masterEditId) {
    ...
    data.ttd = ttdHasNewFile && ttdB64New ? ttdB64New
             : ttdCleared ? '' : (old && old.ttd) ? old.ttd : '';
    data.cap = capHasNewFile && capB64New ? capB64New
             : capCleared ? '' : (old && old.cap) ? old.cap : '';
...
async function savePejabatPengadaan() {
  ...
  try {
    ...
    toast(masterEditId ? 'Pejabat Pengadaan diperbarui' : 'Pejabat Pengadaan ditambahkan', 'success');
    masterEditId = null;
  } catch(e) { toast('Gagal: ' + e.message, 'error'); }
}

// ========== KONFIRMASI HAPUS MASTER ==========
function confirmDeleteMaster(type, id, name) {
  document.getElementById('delete-confirm-msg').innerHTML = `Yakin hapus <strong>${name}</strong>?`;
  ...
  btn.onclick = async () => {
    ...
    try {
      ...
      masterState[type] = await dbGetAll(type);
      renderMaster();
      populateDropdowns();
      if (type === 'bidang' || type === 'rekening') populateDashboardFilters();
...
// Untuk kompatibilitas (jika ada yang memanggil deleteMaster langsung)
async function deleteMaster(store, id) {
  try {
    ...
    renderMaster();
    populateDropdowns();
    if (store === 'bidang' || store === 'rekening') populateDashboardFilters();
...
function parseCSVFull(text) {
  ...
  while (i < text.length) {
    ...
    if (c === ',') { record.push(cur.trim()); cur = ''; i++; continue; }
    if (c === '\r' && text[i+1] === '\n') {
      record.push(cur.trim()); cur = '';
...
async function importCSV() {
  ...
  if (records.length < 2) { toast('File CSV kosong atau tidak valid!', 'error'); return; }
...
function downloadCSVTemplate() {
  const target = document.getElementById('import-csv-target').value;
  const TEMPLATES = {
    paket:    'OPD,RUP,Nama Paket,Program,Kegiatan,Sub Kegiatan,Masa Kerja,Durasi,Tanggal Pesanan,Tanggal Selesai,Pagu Anggaran,Kode Rekening,Bidang,Output\n',
    rincian:  'RUP,Item Barang & Spek,Vol,Satuan,Harga Satuan,User\n',
    harga:    'RUP,Nama Item,Nama Produk,Nama Penyedia,Qty,Satuan,Harga Tayang,Status Pajak,Ongkir,PDN,UMKM,Lokasi,Status Katalog,Nego Final\n',
    penyedia: 'Nama Penyedia,Alamat,Bentuk Usaha,Status,Tipe,Link Toko\n',
  };
  const csv = TEMPLATES[target] || TEMPLATES['paket'];
...
function mapCSVToRecord(target, obj) {
  ...
  const getD = (patterns) => normalizeDate(get(patterns));
  ...
  if (target === 'paket') return {
    ...
    tanggalPesanan: getD(['pesanan','tanggalpesanan']), tanggalSelesai: getD(['selesai','tanggalselesai']),
...
// ============================================================
//  COLLAPSIBLE NAV GROUP — Sidebar Dokumen toggle
//  Primary definition; sidebar-dokumen-dropdown.js boleh override
//  jika valid. Inline guard di index.html jadi jaring pengaman akhir.
// ============================================================
function toggleNavGroup(name) {
  const items  = document.getElementById('nav-group-' + name);
  const toggle = document.getElementById('nav-toggle-' + name);
  if (!items) return;
  const isOpen = items.classList.contains('open');
  // Toggle class (untuk CSS animation jika ada)
  items.classList.toggle('open', !isOpen);
  if (toggle) toggle.classList.toggle('open', !isOpen);
  // Paksa display agar pasti tampil/sembunyi meski CSS tidak handle
  if (!isOpen) {
    ...
    items.style.overflow  = 'hidden';
    setTimeout(function() {
      if (!items.classList.contains('open')) items.style.display = 'none';
    }, 300);
  }
  // Putar arrow indicator
...
// ============================================================
// DISABLED: deadline banner di atas dashboard dinonaktifkan.
// Deadline ditampilkan di dash-bottom-section (bagian bawah dashboard).
function _injectDeadlineBanner() { /* disabled */ }
function renderDeadlineBanner()  { /* disabled */ }
...
// ── Patch renderDashboard — deadline banner dinonaktifkan ─────
// renderDeadlineBanner sudah di-disable; patch ini tetap ada
// agar renderDashboard asli tetap berjalan normal.
...
// ============================================================
//  📦 DASHBOARD v5 — Render bagian bawah Dashboard
//  (Detail Anggaran + Pagu vs Kontrak + Deadline)
//  Menggunakan data state.paket + state.harga yang sudah ada.
// ============================================================
(function(){
  let _chartDashPK = null;

  function _fmt(n){ try { return (typeof fmtRp==='function') ? fmtRp(n) : ('Rp '+Number(n||0).toLocaleString('id-ID')); } catch(e){ return 'Rp 0'; } }

  function _populateDashBotFilters(){
    const paket = (window.state && state.paket && state.paket.data) || [];
    const bSel = document.getElementById('dash-bot-filter-bidang');
    const rSel = document.getElementById('dash-bot-filter-rekening');
    if (bSel && bSel.options.length <= 1){
      const set = [...new Set(paket.map(p=>p.bidang).filter(Boolean))].sort();
      set.forEach(b => { const o=document.createElement('option'); o.value=b; o.textContent=b; bSel.appendChild(o); });
    }
    if (rSel && rSel.options.length <= 1){
      const set = [...new Set(paket.map(p=>p.rekening||p.kodeRekening).filter(Boolean))].sort();
      set.forEach(r => { const o=document.createElement('option'); o.value=r; o.textContent=r; rSel.appendChild(o); });
    }
  }

  window.resetDashBottomFilters = function(){
    const b=document.getElementById('dash-bot-filter-bidang');
    const r=document.getElementById('dash-bot-filter-rekening');
    if(b) b.value=''; if(r) r.value='';
    renderDashBottom();
  };

  window.renderDashBottom = function(){
    if (!document.getElementById('dash-bottom-section')) return;
    _populateDashBotFilters();

    const paket  = (window.state && state.paket && state.paket.data) || [];
    const harga  = (window.state && state.harga && state.harga.data) || [];
    const rincian = (window.state && state.rincian && state.rincian.data) || [];

    const fb = (document.getElementById('dash-bot-filter-bidang')||{}).value || '';
    const fr = (document.getElementById('dash-bot-filter-rekening')||{}).value || '';

    const _paket = paket.filter(p => {
      if (fb && p.bidang !== fb) return false;
      if (fr && (p.rekening||p.kodeRekening) !== fr) return false;
      return true;
    });

    // Hitung agregat
    let totalPagu=0, totalKontrak=0, totalRincian=0;
    _paket.forEach(p=>{
      const pagu = Number(p.paguAnggaran)||0;
      totalPagu += pagu;
      const rup = String(p.rup);
      const hi = harga.filter(h=>String(h.rup)===rup);
      const nego = (typeof _hitungNilaiPenetapan==='function') ? _hitungNilaiPenetapan(rup)
                 : (typeof _nilaiNegoPemenang==='function' ? _nilaiNegoPemenang(hi) : 0);
      totalKontrak += nego;
      const ri = rincian.filter(r=>String(r.rup)===rup);
      ri.forEach(r => { totalRincian += (Number(r.subtotal)||(Number(r.harga)||0)*(Number(r.jumlah)||0)); });
    });
    const efisiensi = totalPagu - totalKontrak;
    const serapan = totalPagu>0 ? (totalKontrak/totalPagu*100) : 0;

    const set = (id,v,cls)=>{ const el=document.getElementById(id); if(!el) return; el.textContent=v; if(cls){ el.classList.remove('pos','neg'); el.classList.add(cls); } };
    set('dbs-paket', _paket.length);
    set('dbs-pagu',     _fmt(totalPagu));
    set('dbs-kontrak',  _fmt(totalKontrak));
    set('dbs-rincian',  _fmt(totalRincian));
    set('dbs-serapan',  serapan.toFixed(1)+'%', serapan>=80?'pos':(serapan>=40?'':'neg'));
    set('dbs-efisiensi', (efisiensi>=0?'+':'')+_fmt(efisiensi), efisiensi>=0?'pos':'neg');
    const cnt=document.getElementById('dash-bot-count'); if(cnt) cnt.textContent=_paket.length+' paket';

    // Chart Pagu vs Kontrak per Bidang
    const byBidang = {};
    _paket.forEach(p=>{
      const b = p.bidang || '— Tidak diisi —';
      if(!byBidang[b]) byBidang[b]={pagu:0,kontrak:0};
      byBidang[b].pagu += Number(p.paguAnggaran)||0;
      const rup=String(p.rup);
      const hi=harga.filter(h=>String(h.rup)===rup);
      const nego=(typeof _hitungNilaiPenetapan==='function')?_hitungNilaiPenetapan(rup):(typeof _nilaiNegoPemenang==='function'?_nilaiNegoPemenang(hi):0);
      byBidang[b].kontrak += nego;
    });
    const labels = Object.keys(byBidang);
    const dPagu = labels.map(l=>byBidang[l].pagu);
    const dKon  = labels.map(l=>byBidang[l].kontrak);

    const ctx = document.getElementById('chart-dash-pagu-kontrak');
    if (ctx && window.Chart){
      // Destroy any existing Chart instance on this canvas (reliable cross-scope)
      const existing = Chart.getChart(ctx);
      if (existing) { try { existing.destroy(); } catch(e){} }
      _chartDashPK = new Chart(ctx, {
        type: 'bar',
        data: { labels, datasets: [
          { label:'Pagu Anggaran', data:dPagu,  backgroundColor:'rgba(99,102,241,.55)',  borderRadius:6 },
          { label:'Nilai Kontrak', data:dKon,   backgroundColor:'rgba(34,197,94,.65)',   borderRadius:6 }
        ]},
        options: {
          responsive:true, maintainAspectRatio:false,
          ...
          scales:{
            ...
            y:{ ticks:{ font:{size:10}, callback:v=>'Rp '+(v/1e6).toFixed(0)+'jt' } }
          }
        }
      });
    }
    ...
    const lbl  = document.getElementById('dd-count-label');
    if (list){
      const today = new Date(); today.setHours(0,0,0,0);
      const items = _paket
        .filter(p=>p.tanggalSelesai)
        .map(p=>{
          const dt = new Date(p.tanggalSelesai);
          const diff = Math.round((dt-today)/86400000);
          return { nama:p.namaPaket||p.rup, diff };
        })
        .sort((a,b)=>a.diff-b.diff)   // terdekat/paling lewat duluan
        .slice(0,18);
      list.innerHTML = items.length
        ? items.map(it=>{
            const up = it.diff>=0;
            const txt = up ? (it.diff+' hari lagi') : ('lewat '+Math.abs(it.diff)+' hari');
            const ico = up ? '⏰' : '🚨';
            return '<div class="dd-item'+(up?' upcoming':'')+'" onclick="showPage(\'paket\')" title="Buka Data Paket">'
                 + '<span class="dd-badge">'+ico+' '+txt+'</span>'
                 + '<span class="dd-name">'+it.nama+'</span></div>';
          }).join('')
        : '<div class="dd-empty">✅ Tidak ada paket yang mendekati / melewati deadline.</div>';
      if (lbl) lbl.textContent = items.length+' item';
    }
  };

  // Hook ke event yang sudah ada
  function _hookDashBottom(){
    if (typeof window.applyDashboardFilters === 'function'){
      const _orig = window.applyDashboardFilters;
      window.applyDashboardFilters = function(){ const r=_orig.apply(this,arguments); try{ renderDashBottom(); }catch(e){} return r; };
    }
    if (typeof window.showPage === 'function'){
      const _orig = window.showPage;
      window.showPage = function(page){ const r=_orig.apply(this,arguments); if(page==='dashboard'){ setTimeout(renderDashBottom, 80); } return r; };
    }
  }

  window.addEventListener('sb-ready',     ()=> setTimeout(()=>{ _hookDashBottom(); renderDashBottom(); }, 800));
  document.addEventListener('DOMContentLoaded', ()=> setTimeout(()=>{ _hookDashBottom(); renderDashBottom(); }, 1500));
})();

// Quick action dari dashboard
function quickAddPaket() {
  // Buka modal tambah paket (pastikan fungsi openAddModal ada)
  if (typeof openAddModal === 'function') {
  openAddModal();
  } else {
    showPage('paket');
    setTimeout(() => document.querySelector('#page-paket .btn-primary')?.click(), 500);
  }
}
function quickInputHarga() {
  showPage('harga');
  setTimeout(() => {
    const addBtn = document.querySelector('#page-harga .btn-primary');
    if (addBtn) addBtn.click();
    else openAddModal('harga');
  }, 500);
}
function renderAuditLog() {
  const el = document.getElementById('audit-log-content');
  if (!el) return;
  el.innerHTML = '<div class="card"><div class="card-body">📋 Riwayat aktivitas akan ditampilkan di sini.</div></div>';
}

function renderErrorLog() {
  const el = document.getElementById('error-log-content');
  if (!el) return;
  el.innerHTML = '<div class="card"><div class="card-body">⚠️ Daftar error sistem akan tampil di sini.</div></div>';
}

function renderHelperPage() {
  const el = document.getElementById('helper-content');
  if (!el) return;
  el.innerHTML = '<div class="card"><div class="card-body">🆘 Panduan dan bantuan cepat akan tersedia di sini.</div></div>';
}

// ========== FUNGSI MODAL UNTUK DATA TRANSAKSIONAL ==========
window.openAddModal = function() {
  const page = currentPage;
  if (page === 'paket') {
    window.editId = null;
    window.editStore = null;
    document.getElementById('modal-paket-title').textContent = 'Tambah Paket';
    // Reset form paket (sesuaikan dengan id form Anda)
    const formIds = ['f-opd', 'f-rup', 'f-namaPaket', 'f-program', 'f-kegiatan', 'f-subKegiatan', 
                     'f-masaKerja', 'f-durasi', 'f-tanggalPesanan', 'f-tanggalSelesai', 
                     'f-paguAnggaran', 'f-kodeRekening', 'f-bidang', 'f-kepalaBidang', 'f-nip', 
                     'f-tanggalDPP', 'f-output'];
    ...
    window.editId = null;
    window.editStore = null;
    document.getElementById('modal-rincian-title').textContent = 'Tambah Rincian';
    // Reset form rincian
    const formIds = ['fr-rup', 'fr-user', 'fr-itemBarang', 'fr-vol', 'fr-satuan', 'fr-hargaSatuan', 'fr-jumlah'];
    formIds.forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
    ...
    // Reset form harga (panjang, sesuaikan)
    const formIds = ['fh-rup', 'fh-namaItem', 'fh-namaProduk', 'fh-namaPenyedia', 'fh-linkKatalog',
                     'fh-qty', 'fh-satuan', 'fh-hargaTayang', 'fh-statusPajak', 'fh-ongkir', 
                     'fh-pdn', 'fh-umkm', 'fh-lokasi', 'fh-statusKatalog', 'fh-negoFinal'];
    formIds.forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
    // Reset otomatis
    if (typeof calcHarga === 'function') calcHarga();
    openModal('modal-harga');
  }
  else if (page === 'penyedia') {
    window.editId = null;
    window.editStore = null;
    document.getElementById('modal-penyedia-title').textContent = 'Tambah Penyedia';
    const formIds = ['fp-nama', 'fp-alamat', 'fp-bentuk', 'fp-status', 'fp-tipe', 'fp-link'];
    formIds.forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
    openModal('modal-penyedia');
  }
  else {
    toast('Halaman ini tidak mendukung tambah data', 'info');
  }
};

window.editRecord = async function(store, id) {
  window.editStore = store;
  window.editId = id;
  
  let data = null;
  if (store === 'paket') data = state.paket.data.find(d => d.id == id);
  else if (store === 'rincian') data = state.rincian.data.find(d => d.id == id);
  else if (store === 'harga') data = state.harga.data.find(d => d.id == id);
  else if (store === 'penyedia') data = state.penyedia.data.find(d => d.id == id);
  ...
  if (store === 'paket') {
    ...
    document.getElementById('f-rup').value = data.rup || '';
    document.getElementById('f-namaPaket').value = data.namaPaket || '';
    document.getElementById('f-program').value = data.program || '';
    document.getElementById('f-kegiatan').value = data.kegiatan || '';
    document.getElementById('f-subKegiatan').value = data.subKegiatan || '';
    document.getElementById('f-masaKerja').value = data.masaKerja || 'Hari Kalender';
    document.getElementById('f-durasi').value = data.durasi || '';
    document.getElementById('f-tanggalPesanan').value = data.tanggalPesanan ? data.tanggalPesanan.split('T')[0] : '';
    document.getElementById('f-tanggalSelesai').value = data.tanggalSelesai ? data.tanggalSelesai.split('T')[0] : '';
    document.getElementById('f-paguAnggaran').value = data.paguAnggaran || '';
    document.getElementById('f-kodeRekening').value = data.kodeRekening || '';
    document.getElementById('f-bidang').value = data.bidang || '';
    document.getElementById('f-kepalaBidang').value = data.kepalaBidang || '';
    document.getElementById('f-nip').value = data.nip || '';
    document.getElementById('f-tanggalDPP').value = data.tanggalDPP ? data.tanggalDPP.split('T')[0] : '';
    document.getElementById('f-output').value = data.output || '';
    openModal('modal-paket');
  }
  else if (store === 'rincian') {
    document.getElementById('modal-rincian-title').textContent = 'Edit Rincian';
    document.getElementById('fr-rup').value = data.rup || '';
    document.getElementById('fr-user').value = data.user || '';
    document.getElementById('fr-itemBarang').value = data.itemBarang || '';
    document.getElementById('fr-vol').value = data.vol || '';
    document.getElementById('fr-satuan').value = data.satuan || '';
    document.getElementById('fr-hargaSatuan').value = data.hargaSatuan || '';
    // jumlah dihitung otomatis
    if (typeof calcJumlah === 'function') calcJumlah();
    openModal('modal-rincian');
  }
  else if (store === 'harga') {
    document.getElementById('modal-harga-title').textContent = 'Edit Survey Harga';
    document.getElementById('fh-rup').value = data.rup || '';
    document.getElementById('fh-namaItem').value = data.namaItem || '';
    document.getElementById('fh-namaProduk').value = data.namaProduk || '';
    document.getElementById('fh-namaPenyedia').value = data.namaPenyedia || '';
    document.getElementById('fh-linkKatalog').value = data.linkKatalog || '';
    document.getElementById('fh-qty').value = data.qty || '';
    document.getElementById('fh-satuan').value = data.satuan || '';
    document.getElementById('fh-hargaTayang').value = data.hargaTayang || '';
    document.getElementById('fh-statusPajak').value = data.statusPajak || 'Tidak Kena Pajak';
    document.getElementById('fh-ongkir').value = data.ongkir || 0;
    document.getElementById('fh-pdn').value = data.pdn || 'Ya';
    document.getElementById('fh-umkm').value = data.umkm || 'Ya';
    document.getElementById('fh-lokasi').value = data.lokasi || 'Kapus Hulu';
    document.getElementById('fh-statusKatalog').value = data.statusKatalog || 'Aktif';
    document.getElementById('fh-negoFinal').value = data.negoFinal || '';
    if (typeof calcHarga === 'function') calcHarga();
    openModal('modal-harga');
  }
  else if (store === 'penyedia') {
    document.getElementById('modal-penyedia-title').textContent = 'Edit Penyedia';
    document.getElementById('fp-nama').value = data.namaPenyedia || '';
    document.getElementById('fp-alamat').value = data.alamat || '';
    document.getElementById('fp-bentuk').value = data.bentukUsaha || 'Perorangan';
    document.getElementById('fp-status').value = data.status || 'Aktif';
    document.getElementById('fp-tipe').value = data.tipe || 'UMKM';
    document.getElementById('fp-link').value = data.linkToko || '';
    openModal('modal-penyedia');
  }
};

window.deleteRecord = function(store, id, name) {
  window._deleteStore = store;
  window._deleteId = id;
  document.getElementById('delete-confirm-msg').innerHTML = `Yakin ingin menghapus <strong>${name || id}</strong>?`;
  document.getElementById('delete-confirm-input').value = '';
  document.getElementById('delete-confirm-btn').disabled = true;
  // Hapus event listener lama dengan mengganti onclick
  const confirmBtn = document.getElementById('delete-confirm-btn');
  const newBtn = confirmBtn.cloneNode(true);
  confirmBtn.parentNode.replaceChild(newBtn, confirmBtn);
  newBtn.onclick = async () => {
    if (document.getElementById('delete-confirm-input').value !== 'HAPUS') return;
    try {
      await dbDelete(store, id);
      await loadAllData();
...
// ============================================================
//  FUNGSI MODAL & AKSI (TAMBAH, EDIT, HAPUS)
// ============================================================
...
// ── Tombol Tambah (dari topbar) ──────────────────────────
window.openAddModal = function() {
  const page = currentPage;
  if (page === 'paket') {
    window.editId = null;
    window.editStore = null;
    document.getElementById('modal-paket-title').innerText = 'Tambah Paket';
    // Reset form paket
    const fields = ['f-opd', 'f-rup', 'f-namaPaket', 'f-program', 'f-kegiatan', 'f-subKegiatan',
                    'f-masaKerja', 'f-durasi', 'f-tanggalPesanan', 'f-tanggalSelesai',
                    'f-paguAnggaran', 'f-kodeRekening', 'f-bidang', 'f-kepalaBidang', 'f-nip',
                    'f-tanggalDPP', 'f-output'];
    ...
    window.editId = null;
    window.editStore = null;
    document.getElementById('modal-rincian-title').innerText = 'Tambah Rincian';
    const fields = ['fr-rup', 'fr-user', 'fr-itemBarang', 'fr-vol', 'fr-satuan', 'fr-hargaSatuan', 'fr-jumlah'];
    fields.forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
    openModal('modal-rincian');
  }
  else if (page === 'harga') {
    window.editId = null;
    window.editStore = null;
    document.getElementById('modal-harga-title').innerText = 'Tambah Survey Harga';
    const fields = ['fh-rup', 'fh-namaItem', 'fh-namaProduk', 'fh-namaPenyedia', 'fh-linkKatalog',
                    'fh-qty', 'fh-satuan', 'fh-hargaTayang', 'fh-statusPajak', 'fh-ongkir',
                    'fh-pdn', 'fh-umkm', 'fh-lokasi', 'fh-statusKatalog', 'fh-negoFinal'];
    fields.forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
    if (typeof calcHarga === 'function') calcHarga();
    ...
    window.editId = null;
    window.editStore = null;
    document.getElementById('modal-penyedia-title').innerText = 'Tambah Penyedia';
    const fields = ['fp-nama', 'fp-alamat', 'fp-bentuk', 'fp-status', 'fp-tipe', 'fp-link'];
    fields.forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
    openModal('modal-penyedia');
  }
  else {
    toast('Halaman ini tidak memiliki form tambah data', 'info');
  }
};
...
window.editRecord = async function(store, id) {
  ...
  if (store === 'paket') {
    document.getElementById('modal-paket-title').innerText = 'Edit Paket';
    document.getElementById('f-opd').value = data.opd || '';
    document.getElementById('f-rup').value = data.rup || '';
    document.getElementById('f-namaPaket').value = data.namaPaket || '';
    document.getElementById('f-program').value = data.program || '';
    ...
    document.getElementById('modal-rincian-title').innerText = 'Edit Rincian';
    document.getElementById('fr-rup').value = data.rup || '';
    document.getElementById('fr-user').value = data.user || '';
    document.getElementById('fr-itemBarang').value = data.itemBarang || '';
    document.getElementById('fr-vol').value = data.vol || '';
    ...
    document.getElementById('modal-harga-title').innerText = 'Edit Survey Harga';
    document.getElementById('fh-rup').value = data.rup || '';
    document.getElementById('fh-namaItem').value = data.namaItem || '';
    document.getElementById('fh-namaProduk').value = data.namaProduk || '';
    document.getElementById('fh-namaPenyedia').value = data.namaPenyedia || '';
    ...
    document.getElementById('modal-penyedia-title').innerText = 'Edit Penyedia';
    document.getElementById('fp-nama').value = data.namaPenyedia || '';
    document.getElementById('fp-alamat').value = data.alamat || '';
    document.getElementById('fp-bentuk').value = data.bentukUsaha || 'Perorangan';
    document.getElementById('fp-status').value = data.status || 'Aktif';
...
window.deleteRecord = function(store, id, name) {
  ...
  const oldBtn = document.getElementById('delete-confirm-btn');
...
// ── Perbaiki juga fungsi renderPaket dll jika kolom kosong ──
// Ini contoh untuk renderPaket (pastikan menggunakan properti yang benar)
// Jika kode renderPaket sudah benar, abaikan. Jika masih kosong, ganti dengan di bawah.
// Tapi karena Anda tidak mengirim kode renderPaket, saya sarankan cek bahwa di renderPaket
// properti yang dipakai adalah namaPaket, paguAnggaran, dll (camelCase).

// ============================================================
//  PERBAIKAN TOMBOL & MODAL (PASTIKAN BERFUNGSI)
// ============================================================

// 1. Pastikan fungsi modal tersedia
...
// 2. Tombol Tambah (mendeteksi halaman aktif dengan benar)
window.openAddModal = function() {
  // Cari halaman aktif dari class .page.active
  const activePage = document.querySelector('.page.active');
  let page = '';
  if (activePage) {
    if (activePage.id === 'page-paket') page = 'paket';
    else if (activePage.id === 'page-rincian') page = 'rincian';
    else if (activePage.id === 'page-harga') page = 'harga';
    else if (activePage.id === 'page-penyedia') page = 'penyedia';
  }
  // Fallback ke variabel currentPage (jika ada)
  if (!page && typeof currentPage !== 'undefined') page = currentPage;
  
  if (page === 'paket') {
    window.editId = null; window.editStore = null;
    document.getElementById('modal-paket-title').innerText = 'Tambah Paket';
    // Reset form sederhana
    const inputs = ['f-opd', 'f-rup', 'f-namaPaket', 'f-program', 'f-kegiatan', 'f-subKegiatan',
                    'f-masaKerja', 'f-durasi', 'f-tanggalPesanan', 'f-tanggalSelesai',
                    'f-paguAnggaran', 'f-kodeRekening', 'f-bidang', 'f-kepalaBidang', 'f-nip',
                    'f-tanggalDPP', 'f-output'];
    ...
    window.editId = null; window.editStore = null;
    document.getElementById('modal-rincian-title').innerText = 'Tambah Rincian';
    const inputs = ['fr-rup', 'fr-user', 'fr-itemBarang', 'fr-vol', 'fr-satuan', 'fr-hargaSatuan', 'fr-jumlah'];
    inputs.forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
    openModal('modal-rincian');
  }
  else if (page === 'harga') {
    window.editId = null; window.editStore = null;
    document.getElementById('modal-harga-title').innerText = 'Tambah Survey Harga';
    const inputs = ['fh-rup', 'fh-namaItem', 'fh-namaProduk', 'fh-namaPenyedia', 'fh-linkKatalog',
                    'fh-qty', 'fh-satuan', 'fh-hargaTayang', 'fh-statusPajak', 'fh-ongkir',
                    'fh-pdn', 'fh-umkm', 'fh-lokasi', 'fh-statusKatalog', 'fh-negoFinal'];
    inputs.forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
    if (typeof calcHarga === 'function') calcHarga();
    ...
    window.editId = null; window.editStore = null;
    document.getElementById('modal-penyedia-title').innerText = 'Tambah Penyedia';
    const inputs = ['fp-nama', 'fp-alamat', 'fp-bentuk', 'fp-status', 'fp-tipe', 'fp-link'];
    inputs.forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
    openModal('modal-penyedia');
  }
  else {
...
// 3. Tombol Edit (sementara buka modal dengan data)
window.editRecord = async function(store, id) {
  window.editStore = store;
  window.editId = id;
  let data = null;
  ...
  if (!data) { toast('Data tidak ditemukan', 'error'); return; }
...
window.deleteRecord = function(store, id, name) {
  ...
  document.getElementById('delete-confirm-input').value = '';
  document.getElementById('delete-confirm-btn').disabled = true;
  // Ganti event listener dengan clone
  const oldBtn = document.getElementById('delete-confirm-btn');
  const newBtn = oldBtn.cloneNode(true);
  oldBtn.parentNode.replaceChild(newBtn, oldBtn);
  newBtn.onclick = async () => {
...
// 5. Refresh tabel jika data tidak tampil (panggil ulang renderAll)
if (typeof renderAll === 'function') {
  setTimeout(renderAll, 500);
}

// ============================================================
//  PERBAIKAN TOMBOL TAMBAH, EDIT, HAPUS UNTUK SEMUA MODUL
// ============================================================

// Pastikan fungsi modal dasar ada
if (typeof window.openModal !== 'function') {
  window.openModal = function(id) {
    const el = document.getElementById(id);
    if (el) el.classList.add('open');
...
// Tombol Tambah untuk data transaksional (Paket, Rincian, Harga, Penyedia, E-Catalog)
window.openAddModal = function() {
  const activePage = document.querySelector('.page.active');
  let page = '';
  if (activePage) {
    ...
    else if (activePage.id === 'page-ecatalog') page = 'ecatalog';
  }
  if (!page && typeof currentPage !== 'undefined') page = currentPage;
  ...
  if (page === 'paket') {
    window.editId = null; window.editStore = null;
    document.getElementById('modal-paket-title').innerText = 'Tambah Paket';
    const inputs = ['f-opd','f-rup','f-namaPaket','f-program','f-kegiatan','f-subKegiatan','f-masaKerja','f-durasi','f-tanggalPesanan','f-tanggalSelesai','f-paguAnggaran','f-kodeRekening','f-bidang','f-kepalaBidang','f-nip','f-tanggalDPP','f-output'];
    inputs.forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
    openModal('modal-paket');
  } 
  else if (page === 'rincian') {
    ...
    window.editId = null; window.editStore = null;
    document.getElementById('modal-harga-title').innerText = 'Tambah Survey Harga';
    const inputs = ['fh-rup','fh-hps','fh-namaPaket','fh-namaItem','fh-namaProduk','fh-linkKatalog','fh-qty','fh-satuan','fh-hargaTayang','fh-statusPajak','fh-ongkir','fh-pdn','fh-umkm','fh-lokasi','fh-statusKatalog','fh-negoFinal'];
    inputs.forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
    document.getElementById('fh-statusPajak').value = 'Tidak Kena Pajak';
    if (typeof calcHarga === 'function') calcHarga();
    if (typeof _buildPenyediaDropdown === 'function') _buildPenyediaDropdown('');
    openModal('modal-harga');
  }
  else if (page === 'penyedia') {
    window.editId = null; window.editStore = null;
  ...
  else if (page === 'ecatalog') {
    if (typeof openEcatalogModal === 'function') openEcatalogModal();
    else toast('Fitur tambah E-Catalog belum siap', 'info');
  }
  else {
    toast('Halaman ini tidak memiliki form tambah', 'info');
  }
};

// Tombol Edit untuk data transaksional
window.editRecord = async function(store, id) {
  window.editStore = store;
  window.editId = id;
  let data = null;
  ...
  else if (store === 'ecatalog') data = masterState.ecatalog.find(d => d.id == id);
  ...
  if (store === 'paket') {
    ...
    document.getElementById('modal-harga-title').innerText = 'Edit Survey Harga';
    document.getElementById('fh-rup').value = data.rup || '';
    document.getElementById('fh-hps').value = data.hps || '';
    document.getElementById('fh-namaPaket').value = data.namaPaket || '';
    document.getElementById('fh-namaItem').value = data.namaItem || '';
    document.getElementById('fh-namaProduk').value = data.namaProduk || '';
    document.getElementById('fh-linkKatalog').value = data.linkKatalog || '';
    document.getElementById('fh-qty').value = data.qty || '';
    ...
    document.getElementById('fh-negoFinal').value = data.negoFinal || '';
    if (typeof calcHarga === 'function') calcHarga();
    if (typeof _buildPenyediaDropdown === 'function') _buildPenyediaDropdown(data.namaPenyedia || '');
    openModal('modal-harga');
  }
  else if (store === 'penyedia') {
    document.getElementById('modal-penyedia-title').innerText = 'Edit Penyedia';
  ...
  else if (store === 'ecatalog') {
    if (typeof editEcatalog === 'function') editEcatalog(id);
    else toast('Edit E-Catalog belum siap', 'info');
  }
};

// Tombol Hapus untuk data transaksional (termasuk E-Catalog)
window.deleteRecord = function(store, id, name) {
  window._deleteStore = store;
  window._deleteId = id;
  document.getElementById('delete-confirm-msg').innerHTML = `Yakin ingin menghapus <strong>${name || id}</strong>?`;
  document.getElementById('delete-confirm-input').value = '';
  document.getElementById('delete-confirm-btn').disabled = true;
  // Clone untuk ganti event listener
  const oldBtn = document.getElementById('delete-confirm-btn');
  const newBtn = oldBtn.cloneNode(true);
  oldBtn.parentNode.replaceChild(newBtn, oldBtn);
  newBtn.onclick = async () => {
    ...
    try {
      if (store === 'ecatalog') {
        await dbDelete('ecatalog', id);
        masterState.ecatalog = await dbGetAll('ecatalog');
        if (typeof renderEcatalog === 'function') renderEcatalog();
      } else {
        await dbDelete(store, id);
        await loadAllData();
        if (typeof renderAll === 'function') renderAll();
...
// Untuk Master Data (Bidang, OPD, Rekening, PPK, Pejabat Pengadaan)
window.openMasterModal = function(type) {
  window.masterEditId = null;
  window.masterEditType = type;
  if (type === 'bidang') {
    document.getElementById('modal-bidang-title').textContent = 'Tambah Bidang';
    document.getElementById('fb-namaBidang').value = '';
    document.getElementById('fb-kodeSurat').value = '';
    document.getElementById('fb-kepalaBidang').value = '';
    document.getElementById('fb-nip').value = '';
    openModal('modal-bidang');
  } else if (type === 'opd') {
    document.getElementById('modal-opd-title').textContent = 'Tambah OPD';
    document.getElementById('fo-namaOPD').value = '';
    openModal('modal-opd');
  } else if (type === 'rekening') {
    document.getElementById('modal-rekening-title').textContent = 'Tambah Kode Rekening';
    document.getElementById('frk-kodeRekening').value = '';
    document.getElementById('frk-linkEKatalog').value = '';
    openModal('modal-rekening');
  } else if (type === 'ppk') {
    document.getElementById('modal-ppk-title').textContent = 'Tambah PPK';
    document.getElementById('fppk-nama').value = '';
    document.getElementById('fppk-nip').value = '';
    document.getElementById('fppk-jabatan').value = '';
    // Reset file uploads (TTD & Cap)
    ['ttd','cap'].forEach(k => {
      const f = document.getElementById(`fppk-${k}-file`);
      if (f) f.value = '';
      const p = document.getElementById(`fppk-${k}-preview`);
      if (p) p.style.display = 'none';
      const b = document.getElementById(`fppk-${k}-b64`);
      if (b) b.value = '';
    });
    openModal('modal-ppk');
  } else if (type === 'pejabatPengadaan') {
    document.getElementById('modal-pejabatPengadaan-title').textContent = 'Tambah Pejabat Pengadaan';
    document.getElementById('fpp-nama').value = '';
    document.getElementById('fpp-nip').value = '';
    document.getElementById('fpp-jabatan').value = '';
    openModal('modal-pejabatPengadaan');
  }
};

window.editMaster = async function(type, id) {
  window.masterEditId = id;
  window.masterEditType = type;
  if (type === 'bidang') {
    const b = masterState.bidang.find(x => x.id === id);
    if (!b) return;
    ...
    if (!r) return;
    document.getElementById('modal-rekening-title').textContent = 'Edit Kode Rekening';
    document.getElementById('frk-kodeRekening').value = r.kodeRekening || '';
    document.getElementById('frk-linkEKatalog').value = r.linkKatalog || r.linkEcatalog || '';
    openModal('modal-rekening');
  } else if (type === 'ppk') {
    const p = masterState.ppk.find(x => x.id === id);
    if (!p) return;
    ...
    document.getElementById('fppk-nip').value = p.nip || '';
    document.getElementById('fppk-jabatan').value = p.jabatan || '';
    // Restore TTD & Cap preview jika ada
    if (p.ttd) showExistingTtd(p.ttd, 'fppk-ttd-file','fppk-ttd-preview','fppk-ttd-b64','fppk-ttd-cur', p.ttdSizeW||120, p.ttdSizeH||55);
    if (p.cap) showExistingTtd(p.cap, 'fppk-cap-file','fppk-cap-preview','fppk-cap-b64','fppk-cap-cur', p.capSizeW||80, p.capSizeH||80);
    openModal('modal-ppk');
  } else if (type === 'pejabatPengadaan') {
...
// Konfirmasi hapus master
window.confirmDeleteMaster = function(type, id, name) {
  document.getElementById('delete-confirm-msg').innerHTML = `Yakin ingin menghapus <strong>"${name}"</strong>?`;
  const inp = document.getElementById('delete-confirm-input');
  const btn = document.getElementById('delete-confirm-btn');
  inp.value = ''; btn.disabled = true;
...
// Pastikan fungsi deleteMaster sudah ada (jika belum, tambahkan)
if (typeof window.deleteMaster !== 'function') {
  window.deleteMaster = async function(store, id) {
    try {
      await dbDelete(store, id);
      masterState[store] = await dbGetAll(store);
      renderMaster();
      ...
      toast('Gagal hapus data: ' + err.message, 'error');
    }
  };
}

// ============================================================
//  PERBAIKAN MASTER DATA (Bidang, OPD, Rekening, PPK, Pejabat)
// ============================================================

// Pastikan variabel global masterEditId dan masterEditType ada
if (typeof masterEditId === 'undefined') var masterEditId = null;
if (typeof masterEditType === 'undefined') var masterEditType = null;

// Modal untuk master
function openMasterModal(type) {
  console.log('openMasterModal dipanggil dengan type:', type);
  masterEditId = null;
  masterEditType = type;
  
  // Tentukan modal ID
  let modalId = '';
  if (type === 'bidang') modalId = 'modal-bidang';
  else if (type === 'opd') modalId = 'modal-opd';
  else if (type === 'rekening') modalId = 'modal-rekening';
  else if (type === 'ppk') modalId = 'modal-ppk';
  else if (type === 'pejabatPengadaan') modalId = 'modal-pejabatPengadaan';
  else if (type === 'ecatalog') {
    if (typeof openEcatalogModal === 'function') openEcatalogModal();
    else toast('Fungsi E-Catalog belum siap', 'error');
    return;
  }
  
  const modal = document.getElementById(modalId);
  if (!modal) {
    toast('Modal ' + modalId + ' tidak ditemukan', 'error');
    return;
  }
  
  // Reset form sesuai type
  if (type === 'bidang') {
    document.getElementById('modal-bidang-title').textContent = 'Tambah Bidang';
    ...
    document.getElementById('modal-rekening-title').textContent = 'Tambah Kode Rekening';
    clearForm(['frk-kodeRekening', 'frk-linkEKatalog']);
  } 
  else if (type === 'ppk') {
    document.getElementById('modal-ppk-title').textContent = 'Tambah PPK';
    ...
    // Reset TTD & Cap
    ['ttd','cap'].forEach(k => {
      const fileInput = document.getElementById(`fppk-${k}-file`);
      if (fileInput) fileInput.value = '';
      const b64 = document.getElementById(`fppk-${k}-b64`);
      if (b64) b64.value = '';
      const preview = document.getElementById(`fppk-${k}-preview`);
      if (preview) preview.style.display = 'none';
      const cur = document.getElementById(`fppk-${k}-cur`);
  ...
  // FORCE tampilkan modal dengan style langsung
...
async function editMaster(type, id) {
  ...
  if (type === 'bidang') {
    ...
    document.getElementById('fppk-nip').value = p.nip || '';
    document.getElementById('fppk-jabatan').value = p.jabatan || '';
    // Restore TTD & Cap (fungsi showExistingTtd harus ada)
    if (typeof showExistingTtd === 'function') {
      if (p.ttd) showExistingTtd(p.ttd, 'fppk-ttd-file', 'fppk-ttd-preview', 'fppk-ttd-b64', 'fppk-ttd-cur', p.ttdSizeW||120, p.ttdSizeH||55);
      if (p.cap) showExistingTtd(p.cap, 'fppk-cap-file', 'fppk-cap-preview', 'fppk-cap-b64', 'fppk-cap-cur', p.capSizeW||80, p.capSizeH||80);
    }
    openModal('modal-ppk');
...
// Pastikan openAddModal bekerja untuk semua halaman termasuk penyedia
window.openAddModal = function() {
  const activePage = document.querySelector('.page.active');
  let page = '';
  if (activePage) {
  ...
  if (page === 'paket') {
    ...
    document.getElementById('modal-penyedia-title').textContent = 'Tambah Penyedia';
    clearForm(['fp-nama', 'fp-alamat', 'fp-bentuk', 'fp-status', 'fp-tipe', 'fp-link']);
    openModal('modal-penyedia');
  } else if (page === 'ecatalog') {
    if (typeof openEcatalogModal === 'function') openEcatalogModal();
    else toast('Fungsi E-Catalog belum siap', 'error');
...
// ============================================================
//  PERBAIKAN FILTER RINCIAN & HARGA (manual event listener)
// ============================================================
(function fixFilters() {
  // Filter Rincian
  const rincianFilters = [
    'search-rincian', 'filter-rup-rincian', 'filter-rupdd-rincian',
    'filter-satuan-rincian', 'filter-tahun-rincian'
  ];
  rincianFilters.forEach(id => {
    const el = document.getElementById(id);
    if (el && !el.hasAttribute('data-filter-listener')) {
      el.setAttribute('data-filter-listener', 'true');
      const eventType = (el.tagName === 'SELECT' || id.includes('rupdd')) ? 'change' : 'input';
      el.addEventListener(eventType, function() {
        console.log('Filter rincian:', id);
        filterRincian();
      });
    }
  });
  ...
  // Filter Harga
  const hargaFilters = [
    'search-harga', 'filter-rup-harga', 'filter-rupdd-harga',
    'filter-status-harga', 'filter-tahun-harga'
  ];
  hargaFilters.forEach(id => {
    const el = document.getElementById(id);
    if (el && !el.hasAttribute('data-filter-listener')) {
      el.setAttribute('data-filter-listener', 'true');
      const eventType = (el.tagName === 'SELECT' || id.includes('rupdd')) ? 'change' : 'input';
      el.addEventListener(eventType, function() {
        console.log('Filter harga:', id);
        filterHarga();
      });
    }
  });
...
// Pastikan dropdown RUP rincian memicu filter saat berubah (fallback)
document.addEventListener('DOMContentLoaded', function() {
  const rupSelect = document.getElementById('filter-rupdd-rincian');
  if (rupSelect && !rupSelect.hasAttribute('data-filter-bound')) {
    rupSelect.setAttribute('data-filter-bound', 'true');
    rupSelect.addEventListener('change', function() { filterRincian(); });
    console.log('Event listener dipasang pada filter-rupdd-rincian');
  }
});

// ============================================================
//  PERBAIKAN FILTER DROPDOWN RUP DI RINCIAN DAN HARGA
// ============================================================
(function fixRupDropdown() {
  // Fungsi untuk memicu filter dengan nilai dropdown
  function onRincianDropdownChange() {
    const val = this.value;
    console.log('🔽 Dropdown Rincian berubah:', val);
    // Simpan nilai ke input teks agar sinkron (opsional)
    const rupInput = document.getElementById('filter-rup-rincian');
    if (rupInput) rupInput.value = val;
    filterRincian();
  }
  
  function onHargaDropdownChange() {
    const val = this.value;
    console.log('🔽 Dropdown Harga berubah:', val);
    const rupInput = document.getElementById('filter-rup-harga');
    if (rupInput) rupInput.value = val;
    filterHarga();
  }
  
  // Pasang event listener ke dropdown rincian
  const rupRincian = document.getElementById('filter-rupdd-rincian');
  if (rupRincian) {
    // Hapus listener lama dengan clone
    const newRup = rupRincian.cloneNode(true);
    rupRincian.parentNode.replaceChild(newRup, rupRincian);
    newRup.addEventListener('change', onRincianDropdownChange);
    console.log('✅ Event listener dipasang untuk filter-rupdd-rincian');
  }
  
  // Pasang event listener ke dropdown harga
  const rupHarga = document.getElementById('filter-rupdd-harga');
  if (rupHarga) {
    const newRup = rupHarga.cloneNode(true);
    rupHarga.parentNode.replaceChild(newRup, rupHarga);
    newRup.addEventListener('change', onHargaDropdownChange);
    console.log('✅ Event listener dipasang untuk filter-rupdd-harga');
  }
  
  // Pastikan dropdown terisi dengan benar
...
// ============================================================
//  SIMPAN & PULIHKAN FILTER RINCIAN & HARGA (localStorage)
// ============================================================
(function persistFilters() {
  // Daftar ID elemen filter untuk Rincian & Harga
  const filterIds = {
    rincian: ['search-rincian', 'filter-rupdd-rincian', 'filter-rup-rincian', 'filter-satuan-rincian', 'filter-tahun-rincian'],
    harga:   ['search-harga', 'filter-rupdd-harga', 'filter-rup-harga', 'filter-status-harga', 'filter-tahun-harga']
  };

  // Simpan nilai filter ke localStorage (dipanggil setiap filter berubah)
  function saveFilters() {
    const data = {};
    for (const [page, ids] of Object.entries(filterIds)) {
      data[page] = {};
      ids.forEach(id => {
        const el = document.getElementById(id);
        if (el) data[page][id] = el.value;
      });
    }
    localStorage.setItem('sideva_filters', JSON.stringify(data));
  }

  // Pulihkan nilai filter dari localStorage
  function restoreFilters() {
    const saved = localStorage.getItem('sideva_filters');
    if (!saved) return;
    try {
      ...
      for (const [page, ids] of Object.entries(filterIds)) {
        if (data[page]) {
          ids.forEach(id => {
            if (data[page][id] !== undefined) {
              const el = document.getElementById(id);
              if (el) el.value = data[page][id];
            }
          });
        }
      }
  ...
  // Pasang event listener ke semua filter untuk menyimpan perubahan
  function bindSaveEvents() {
    const allIds = [...filterIds.rincian, ...filterIds.harga];
    allIds.forEach(id => {
      const el = document.getElementById(id);
      if (!el || el.hasAttribute('data-persist-bound')) return;
      el.setAttribute('data-persist-bound', 'true');
      const eventType = (el.tagName === 'SELECT') ? 'change' : 'input';
      el.addEventListener(eventType, () => {
        setTimeout(saveFilters, 50);
      });
    });
  }

  // Override filterRincian & filterHarga asli agar setelah selesai, filter tetap tersimpan
  const originalFilterRincian = window.filterRincian;
  const originalFilterHarga = window.filterHarga;
  if (originalFilterRincian) {
    window.filterRincian = function() {
      originalFilterRincian();
      saveFilters();
    };
  }
  if (originalFilterHarga) {
    window.filterHarga = function() {
      originalFilterHarga();
      saveFilters();
    };
  }

  // Inisialisasi: pulihkan filter setelah data siap, lalu simpan saat halaman ditutup
  window.addEventListener('sb-ready', () => {
    setTimeout(() => {
      restoreFilters();
      // Setelah filter dipulihkan, jalankan filter ulang
  ...
  // Simpan filter sebelum halaman di-reload/unload
  window.addEventListener('beforeunload', saveFilters);
})();

// ========== UPDATE SIDEBAR OPD NAME ==========
function updateSidebarOpdName() {
  let opdName = '';
  try {
    if (typeof getCurrentOpdName === 'function') opdName = getCurrentOpdName();
    else if (window._currentOpdId && window._userOpdList) {
      const found = window._userOpdList.find(o => o.id === window._currentOpdId);
      if (found) opdName = found.namaOpd || found.nama;
    }
  } catch(e) {}
  const brandEl = document.getElementById('brand-instansi');
  if (brandEl) {
    if (opdName && opdName !== 'Semua OPD') brandEl.textContent = opdName;
    else if (appConfig && appConfig.singkatan) brandEl.textContent = appConfig.singkatan;
    else brandEl.textContent = 'Instansi Pemerintah';
  }
  // Update footer juga
  const footerEl = document.getElementById('footer-instansi');
  if (footerEl && opdName && opdName !== 'Semua OPD') {
    footerEl.textContent = `SI-DEVA v5 · ${opdName}`;
  }
}

// Panggil sekali saat load
window.addEventListener('sb-ready', () => setTimeout(updateSidebarOpdName, 500));
window.addEventListener('opd-changed', () => setTimeout(updateSidebarOpdName, 100));

function refreshKopPreviewArea() {
  console.log('🖼️ refreshKopPreviewArea dipanggil');
  const area = document.getElementById('kop-preview-area');
  if (area) area.innerHTML = kopSurat();
  const lbl = document.getElementById('kop-preview-label-text');
  if (lbl) {
    const uploadedKop = localStorage.getItem('sideva_kop_surat_img');
    lbl.textContent = uploadedKop
      ? 'Pratinjau kop surat — menggunakan gambar yang diupload'
      : 'Pratinjau kop surat (teks fallback) — berdasarkan data yang diisi di atas';
  }
  const savedImg = localStorage.getItem('sideva_kop_surat_img');
  if (savedImg && typeof renderKopUploadPreview === 'function') {
    renderKopUploadPreview(savedImg, null);
  }
}
