// ============================================================
//  SI-DEVA — Auth UI & Role Management v8
//  (Fix: hilangkan panel user mgmt di backup, perbaiki sintaks)
// ============================================================
...
(function injectAuthStyle() {
  ...
  s.textContent = `
    ...
    .sidebar-logout {
      ...
      border: 1px solid rgba(248,113,113,.22); color: #fca5a5;
    ...
    #user-mgmt-panel th, #user-mgmt-panel td { padding: 8px 10px; border-bottom: 1px solid var(--border,#2a2a2a); }
...
function applyRoleUI() {
  ...
  // Viewer: sembunyikan tombol tulis
  ...
  writeActions.forEach(btn => { btn.style.display = isOperatorRole ? '' : 'none'; });

  // Master data: hanya admin (termasuk admin biasa)
  ...
  masterActions.forEach(btn => { btn.style.display = isAdminRole ? '' : 'none'; });

  // Import & Reset: hanya admin (termasuk admin biasa)
  ...
  adminOnly.forEach(el => { el.style.display = isAdminRole ? '' : 'none'; });

  // Menu Import di sidebar: sembunyikan untuk viewer
  document.querySelectorAll('.nav-item').forEach(btn => {
    const oc = btn.getAttribute('onclick') || '';
    if (oc.includes("'import'") && !isAdminRole) btn.style.display = 'none';
  });

  // Panel manajemen user (di backup) – sembunyikan SELALU karena sudah ada menu terpisah
  const mgmtPanel = document.getElementById('user-mgmt-panel');
  if (mgmtPanel) mgmtPanel.style.display = 'none';

  // --- Menu khusus Super Admin ---
  const superOnlyMenuIds = [
    'nav-manajemen-opd', 
  ...
  if (isSuper) {
    ...
    document.querySelectorAll('.admin-only').forEach(el => {
      if (!superOnlyMenuIds.includes(el.id)) {
        el.classList.add('sideva-admin-show', 'sideva-visible');
      }
    });
  }
  ...
  if (!isAdminRole) cloudBtns.forEach(b => {
    if (!b.onclick?.toString().includes('Logout')) b.style.display = 'none';
  });
}
...
// ── Panel manajemen user (tidak dipakai di backup, tapi fungsi tetap untuk keperluan lain) ──
async function renderUserMgmt() {
  ...
  try {
    ...
    if (!users.length) { panel.innerHTML = '<p style="opacity:.5;font-size:13px;">Belum ada user.</p>'; return; }
    panel.innerHTML = `
...
// ── Inject panel auth ke halaman Backup (TANPA panel user mgmt) ──
function injectAuthPanel() {
  ...
  mount.innerHTML = `
    ...
    <p style="font-size:11px;opacity:.4;margin:8px 0 0;">Data otomatis diperbarui setiap 300 detik.</p>
  `;
}
